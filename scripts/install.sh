...
download_uri=https://nightly.link/shdwmtr/millennium/actions/runs/13217379955/millennium-linux.zip
...
if file $tar | grep -q "gzip compressed data"; then
    ...
    unzip -qo "$tar" -d "$extract_path"
else
    echo "The file is neither a valid tar.gz nor ZIP archive."
fi
