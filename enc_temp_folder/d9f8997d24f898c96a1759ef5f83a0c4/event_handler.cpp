...
void Initialize()
{
    ...
    c_threads::get().add(std::thread([&] {
        ...
        while (should_run)
        {
            ...
            addr_port ps;
            bool is_steam_socket = ps.is_steam_socket(8080, plist);

            if (!is_steam_socket) {

                std::string message = "The following apps were found running on port 8080, "
                    "a port that steam needs for millennium to function. "
                    "You must either close, removed the app, or uninstall millennium.\n\n";
                ...
                msg::show(message.c_str(), "Fatal Bootstrap Error :(", Style::Error);
                break;
            }
#endif
            ...
            try {
                ...
                c.set_close_handler(bind([&](ws_Client* c, websocketpp::connection_hdl hdl) -> void {
                    console.err("socket closed, steam shutdown?");
                    should_run = false;
                }, &c, ::_1));
                ...
                std::string message = fmt::format("Error getting Steams webSocketDebuggerUrl: {}", error.message());
                
                auto selection = msg::show(message.c_str(), "Bootstrap Error");
            }
            catch (websocketpp::exception const& e) {
