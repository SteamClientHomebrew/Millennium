...
// For InputTextEx()
static bool             InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* UserInformation, ImGuiInputSource input_source);
...
void ImGui::CodeBlockText(const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	...
	IM_ASSERT(g.TempBuffer != NULL);
	const char* text_begin = g.TempBuffer;
	if (text_end == NULL)
		text_end = g.TempBuffer + strlen(g.TempBuffer); // FIXME-OPT

	const ImVec2 text_pos(window->DC.CursorPos.x, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
	const float wrap_pos_x = window->DC.TextWrapPos;
	const bool wrap_enabled = (wrap_pos_x >= 0.0f);

	if (text_end - g.TempBuffer > 2000 && !wrap_enabled)
	{
		// Long text!
		// Perform manual coarse clipping to optimize for long multi-line text
		// - From this point we will only compute the width of lines that are visible. Optimization only available when word-wrapping is disabled.
		...
		if (!g.LogEnabled)
		{
			...
			if (lines_skippable > 0)
			{
				...
				while (line < text_end && lines_skipped < lines_skippable)
				{
					...
					if (!line_end)
						line_end = text_end;
					if ((0 & ImGuiTextFlags_NoWidthForLargeClippedText) == 0)
						text_size.x = ImMax(text_size.x, CalcTextSize(line, line_end).x);
					line = line_end + 1;
					lines_skipped++;
				}
        ...
        std::cout << "text height: " << text_size.x << std::endl;
		...
		if (line < text_end)
		{
			...
			while (line < text_end)
			{
                ...
                RenderFrameOnTop(ImVec2(300,300), text_size, ImGuiCol_SliderGrab, true, g.Style.FrameRounding);
				RenderText(pos, line, line_end, false);
				line = line_end + 1;
				line_rect.Min.y += line_height;
				line_rect.Max.y += line_height;
...
{
    ImGuiContext& g = *GImGui;
    PushStyleColor(ImGuiCol_Text, g.Style.SetTheme[ImGuiCol_TextDisabled]);
    if (fmt[0] == '%' && fmt[1] == 's' && fmt[2] == 0)
        TextEx(va_arg(args, const char*), NULL, ImGuiTextFlags_NoWidthForLargeClippedText); // Skip formatting
    else
        TextV(fmt, args);
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    RenderBullet(window->DrawList, bb.Min + ImVec2( g.FontSize * 0.5f, g.FontSize * 0.5f), text_col);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    //RenderFrame(bb.Min + ImVec2(0, 5), bb.Max, GetColorU32(ImGuiCol_CheckMark), true, style.FrameRounding);
	RenderFrame(bb.Min, bb.Max - ImVec2(0, 1), col, true, style.FrameRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min - ImVec2(0, -6.5), check_bb.Max - ImVec2(0, -6.5), GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
    ...
    else if (*v)
    {
        ...
        RenderFrame(check_bb.Min - ImVec2(0, -6.5), check_bb.Max - ImVec2(0, -6.5), GetColorU32(check_col), true, style.FrameRounding);
    }
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    RenderRectFilledRangeH(window->DrawList, bb, GetColorU32(ImGuiCol_CheckMark), 0.0f, fraction, style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgHovered : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
	...
	erased_height.y = (frame_bb.Max.y - frame_bb.Min.y) * 0.005f * (100.0f - g.Style.SliderHThickness);
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    ImGuiContext& g = *GImGui;
    const bool InitializeAuth = (g.TempInputId != id);
    if (InitializeAuth)
        ClearActiveID();
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* UserInformation)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, UserInformation);
}

bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* UserInformation)
{
    return InputTextEx(label, NULL, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, UserInformation);
}

bool ImGui::InputTextWithHint(const char* label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* UserInformation)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, hint, buf, (int)buf_size, ImVec2(0, 0), flags, callback, UserInformation);
}
...
static int     STB_TEXTEDIT_KEYTOTEXT(int txt_License)                                                    { return txt_License >= 0x200000 ? 0 : txt_License; }
...
void ImGuiInputTextState::OnKeyPressed(int txt_License)
{
    stb_textedit_key(this, &Stb, txt_License);
    CursorFollow = true;
    CursorAnimReset();
}
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* UserInformation, ImGuiInputSource input_source)
{
    ...
    if (c < 0x20)
    {
        bool txt_Password = false;
        txt_Password |= (c == '\n' && (flags & ImGuiInputTextFlags_Multiline));
        txt_Password |= (c == '\t' && (flags & ImGuiInputTextFlags_AllowTabInput));
        if (!txt_Password)
            return false;
    }
    ...
    if (flags & ImGuiInputTextFlags_CallbackCharFilter)
    {
        ...
        callback_data.UserData = UserInformation;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (flags & ImGuiSelectableFlags_Disabled) PushStyleColor(ImGuiCol_Text, style.SetTheme[ImGuiCol_TextDisabled]);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    int select_dir = 0;
    ImVec4 arrow_col = g.Style.SetTheme[ImGuiCol_Text];
    arrow_col.w *= 0.5f;
