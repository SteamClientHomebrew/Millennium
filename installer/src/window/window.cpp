...
void initTextures()
{
    createRawImage(&icons_struct.planet, Memory::millennium_icon, sizeof Memory::millennium_icon);
...
bool Application::Create(std::function<void()> Handler, HINSTANCE hInstance)
{
    ...
    overlay.hwnd = CreateWindowExW(
        ...
        appinfo.width, appinfo.height, nullptr, nullptr, nullptr, nullptr
    );
    ...
    if (ImGui_ImplWin32_Init(overlay.hwnd) && ImGui_ImplDX9_Init(directx9.device))
    {
        ...
        while (directx9.msg.message != WM_QUIT)
        {
            ...
            ImGui::NewFrame();
            {
                static ImGuiWindowFlags flags = /*ImGuiWindowFlags_NoDecoration | */ ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings;

                ImGui::SetNextWindowPos(ImGui::GetMainViewport()->WorkPos);
                ImGui::SetNextWindowSize(ImGui::GetMainViewport()->WorkSize);

                ImGui::Begin("###", &g_windowOpen, flags); Handler(); ImGui::End();
            }
            ImGui::EndFrame();
...
void set_proc_theme_colors()
{
    ...
    sty->SetTheme[ImGuiCol_TitleBg] = ImVec4(0.10f, 0.10f, 0.10f, 1.00f);
    sty->SetTheme[ImGuiCol_TitleBgCollapsed] = ImVec4(0.14f, 0.14f, 0.14f, 0.75f);
    sty->SetTheme[ImGuiCol_TitleBgActive] = ImVec4(0.10f, 0.10f, 0.10f, 1.00f);
