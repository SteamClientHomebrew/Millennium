...
pub fn sync_types(plugin_dir: &Path, starlight_version: &str) -> anyhow::Result<()> {
    ...
    if client_zip.exists() {
        ...
        crate::log::warn(&format!(
            "lsp: types.zip not found at {}, skipping",
            client_zip.display()
        ));
    }
    ...
    if webkit_zip.exists() {
        ...
        crate::log::warn(&format!(
            "lsp: webkit-types.zip not found at {}, skipping",
            webkit_zip.display()
        ));
    }
...
pub fn install_types(
    plugin_dir: &Path,
    starlight_version: &str,
    cfg: &crate::config::PlgConfig,
) -> anyhow::Result<()> {
    sync_types(plugin_dir, starlight_version)?;
