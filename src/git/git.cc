...
namespace DistributedGit
{
    const bool CloneRepository(std::string repositoryPath, std::string gitRemoteUrl)
    {
        ...
        git_libgit2_init();
        git_repository* repo = nullptr;
        int repositoryOpenStatus = git_repository_open(&repo, repositoryPath.c_str());

        if (repositoryOpenStatus != GIT_ENOTFOUND) 
        {
            Logger.LogItem("error", fmt::format("failed to clone repo (one already exists) -> {}", git_error_last()->message));
            return false;
        }

        const int cloneSuccess = Dependencies::CloneRepository(repo, repositoryPath, gitRemoteUrl);

        // Free resources
        git_repository_free(repo);
        git_libgit2_shutdown();
        return cloneSuccess == 0;
    }
    ...
    const bool UpdateRepository(std::string packageLocalPath)
    {
        ...
        git_libgit2_init();
        git_repository* repo = nullptr;
        int repositoryOpenStatus = git_repository_open(&repo, packageLocalPath.c_str());

        if (repositoryOpenStatus < 0) 
        {
            Logger.LogItem("error", fmt::format("failed to open repo -> {}", git_error_last()->message));
            return false;
        }

        const int fetchHeadRes = Dependencies::FetchHead(repo, packageLocalPath);

        // Free resources
        git_repository_free(repo);
        git_libgit2_shutdown();
        return fetchHeadRes == 0;
    }
    ...
    const std::string GetLocalCommit(std::string packageLocalPath)
    {
        ...
        git_libgit2_init();
        git_repository *repo = nullptr;
        int errorCode = git_repository_open(&repo, packageLocalPath.c_str());
        ...
        errorCode = git_repository_head(&headRef, repo);
        if (errorCode < 0) 
        {
        ...
        const git_oid *referenceTarget = git_reference_target(headRef);
        if (referenceTarget == nullptr) 
        {
            LOG_ERROR("failed to get commit ID -> {}", git_error_last()->message);
