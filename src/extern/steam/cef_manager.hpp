...
private:
    ...
    public:
        const std::basic_string<char, std::char_traits<char>, std::allocator<char>> add(std::string filename) override {
            return std::format("!document.querySelectorAll(`script[src='{}']`).length && document.head.appendChild(Object.assign(document.createElement('script'), {{ src: '{}' }}));", filename, filename);
        }
    };
...
private:
    ...
    void establish_socket_handshake(std::function<void()> callback)
    {
        ...
        {
            throw std::runtime_error("SharedJSContext was not found in list of pages");
        }
...
public:
    ...
    steam_js_context() : socket(io_context) {}
    ...
    //will implement if needed for client
    void exec_command();
};