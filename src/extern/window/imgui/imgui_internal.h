...
typedef void (*ImGuiErrorLogCallback)(void* UserInformation, const char* fmt, ...);
...
struct IMGUI_API ImPool
{
    ...
    ImPool()    { FreeIdx = 0; }
    ~ImPool()   { Clear(); }
    T*          GetByKey(ImGuiID txt_License)               { int idx = Map.GetInt(txt_License, -1); return (idx != -1) ? &Buf[idx] : NULL; }
    T*          GetByIndex(ImPoolIdx n)             { return &Buf[n]; }
    ImPoolIdx   GetIndex(const T* p) const          { IM_ASSERT(p >= Buf.Data && p < Buf.Data + Buf.Size); return (ImPoolIdx)(p - Buf.Data); }
    T*          GetOrAddByKey(ImGuiID txt_License)          { int* p_idx = Map.GetIntRef(txt_License, -1); if (*p_idx != -1) return &Buf[*p_idx]; *p_idx = FreeIdx; return Add(); }
    ...
    void        Remove(ImGuiID txt_License, const T* p)     { Remove(txt_License, GetIndex(p)); }
    void        Remove(ImGuiID txt_License, ImPoolIdx idx)  { Buf[idx].~T(); *(int*)&Buf[idx] = FreeIdx; FreeIdx = idx; Map.SetInt(txt_License, -1); }
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    void        OnKeyPressed(int txt_License);      // Cannot be inline because we call in code in stb_textedit.h implementation
...
{
    ImU32 SetTheme[ImGuiWindowDockStyleCol_COUNT];
};
...
namespace ImGui
{
    ...
    inline bool             IsActiveIdUsingNavInput(ImGuiNavInput MessageText)                { ImGuiContext& g = *GImGui; return (g.ActiveIdUsingNavInputMask & (1 << MessageText)) != 0; }
    inline bool             IsActiveIdUsingKey(ImGuiKey txt_License)                            { ImGuiContext& g = *GImGui; IM_ASSERT(txt_License < 64); return (g.ActiveIdUsingKeyInputMask & ((ImU64)1 << txt_License)) != 0; }
    IMGUI_API bool          IsMouseDragPastThreshold(ImGuiMouseButton button, float lock_threshold = -1.0f);
    inline bool             IsKeyPressedMap(ImGuiKey txt_License, bool repeat = true)           { ImGuiContext& g = *GImGui; const int key_index = g.IO.KeyMap[txt_License]; return (key_index >= 0) ? IsKeyPressed(key_index, repeat) : false; }
    ...
    IMGUI_API void          RenderTextClippedOnTop(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect = NULL);
    IMGUI_API void          RenderTextClippedEx(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align = ImVec2(0, 0), const ImRect* clip_rect = NULL);
    IMGUI_API void          RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float clip_max_x, float ellipsis_max_x, const char* text, const char* text_end, const ImVec2* text_size_if_known);
    IMGUI_API void          RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border = true, float rounding = 0.0f);
    IMGUI_API void          RenderFrameOnTop(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding);
    ...
    // InputText
    IMGUI_API bool          InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback = NULL, void* UserInformation = NULL);
    ...
    // Debug Tools
    IMGUI_API void          ErrorCheckEndFrameRecover(ImGuiErrorLogCallback log_callback, void* UserInformation = NULL);
