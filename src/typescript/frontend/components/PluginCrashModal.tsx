...
const CrashModal = ({ detail, closeModal, onResolve }: { detail: PluginCrashInfo; closeModal: () => void; onResolve: () => void }): React.ReactElement => {
	...
	return (
		<ConfirmModal
			...
			strTitle={`Not our finest moment. 😅`}
			strDescription={
				<>
					<Field
						description={
							<>
								The <strong>{pluginLabel}</strong> plugin stopped unexpectedly — this is almost certainly a bug in the plugin, not Steam or Millennium. If it
								keeps happening, find the plugin in the{' '}
								<a href={`https://github.com/SteamClientHomebrew/PluginDatabase/tree/main/plugins`} target="_blank" rel="noopener noreferrer">
									Plugin Database
								</a>{' '}
								and report the issue directly to its own repository — not the database itself. Check existing issues first to avoid duplicates, and include the
								exit code and crash files from the dump below.
							</>
						}
						bottomSeparator="none"
					/>

					<SettingsDialogSubHeader>Developer Information</SettingsDialogSubHeader>
					<Field
						label="Exit Code"
						description="The exit code reported by the process. Include this when reporting the bug."
					...
					<Field
						label="Crash Dump"
						description="The following folder contains crash dumps that may have been generated by the plugin. Include any relevant crash dumps when reporting the bug."
					>
						{hasCrashDir && (
					...
					<Field
						label="Plugin Logs"
						description="View the plugin's log output for additional context on what happened before the crash."
						bottomSeparator="none"
					>
			...
			strOKButtonText="Restart Plugin"
			strMiddleButtonText="Disable Plugin"
