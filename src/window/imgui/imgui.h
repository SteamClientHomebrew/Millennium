...
typedef void* (*ImGuiMemAllocFunc)(size_t sz, void* UserInformation);             // Function signature for ImGui::SetAllocatorFunctions()
typedef void (*ImGuiMemFreeFunc)(void* ptr, void* UserInformation);               // Function signature for ImGui::SetAllocatorFunctions()
...
namespace ImGui
{
    ...
    IMGUI_API void          CodeBlockText(const char* fmt, ...)                                IM_FMTARGS(1); // formatted text
    ...
    IMGUI_API void          TextWrappedUnFormatted(const char* fmt, const char* text_end, ...) IM_FMTARGS(1);
    IMGUI_API void          TextWrappedUnFormattedV(const char* fmt, const char* text_end, va_list args) IM_FMTARGS(1);
    ...
    IMGUI_API bool          InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* UserInformation = NULL);
    IMGUI_API bool          InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size = ImVec2(0, 0), ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* UserInformation = NULL);
    IMGUI_API bool          InputTextWithHint(const char* label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* UserInformation = NULL);
    ...
    IMGUI_API void          SetAllocatorFunctions(ImGuiMemAllocFunc alloc_func, ImGuiMemFreeFunc free_func, void* UserInformation = NULL);
...
struct ImGuiStyle
{
	...
	float       SliderHThickness;           // Tab thickness in horizontal sliders. Ratio of the rectangle witdh. Can be set between 0.0f (rectangle witdh is null) and 1.0f (full rectangle is drawn), 
	float       SliderVThickness;           // Tab thickness in vertical sliders. Ratio of the rectangle witdh. Can be set between 0.0f (rectangle witdh is null) and 1.0f (full rectangle is drawn), 
    ...
    ImVec4      SetTheme[ImGuiCol_COUNT];
...
struct ImGuiIO
{
    ...
    const char* (*GetClipboardTextFn)(void* UserInformation);
    void        (*SetClipboardTextFn)(void* UserInformation, const char* text);
...
struct ImGuiStorage
{
    ...
    struct ImGuiStoragePair
    {
        ...
        ImGuiStoragePair(ImGuiID _key, int _val_i)      { txt_License = _key; val_i = _val_i; }
        ImGuiStoragePair(ImGuiID _key, float _val_f)    { txt_License = _key; val_f = _val_f; }
        ImGuiStoragePair(ImGuiID _key, void* _val_p)    { txt_License = _key; val_p = _val_p; }
    };
    ...
    IMGUI_API int       GetInt(ImGuiID txt_License, int default_val = 0) const;
    IMGUI_API void      SetInt(ImGuiID txt_License, int val);
    IMGUI_API bool      GetBool(ImGuiID txt_License, bool default_val = false) const;
    IMGUI_API void      SetBool(ImGuiID txt_License, bool val);
    IMGUI_API float     GetFloat(ImGuiID txt_License, float default_val = 0.0f) const;
    IMGUI_API void      SetFloat(ImGuiID txt_License, float val);
    IMGUI_API void*     GetVoidPtr(ImGuiID txt_License) const; // default_val is NULL
    IMGUI_API void      SetVoidPtr(ImGuiID txt_License, void* val);

    // - Get***Ref() functions finds pair, insert on demand if missing, return pointer. Useful if you intend to do Get+Set.
    // - References are only valid until a new value is added to the storage. Calling a Set***() function or a Get***Ref() function invalidates the pointer.
    // - A typical use case where this is convenient for quick hacking (e.g. add storage during a live Edit&Continue session if you can't modify existing struct)
    //      float* pvar = ImGui::GetFloatRef(key); ImGui::SliderFloat("var", pvar, 0, 100.0f); some_var += *pvar;
    IMGUI_API int*      GetIntRef(ImGuiID txt_License, int default_val = 0);
    IMGUI_API bool*     GetBoolRef(ImGuiID txt_License, bool default_val = false);
    IMGUI_API float*    GetFloatRef(ImGuiID txt_License, float default_val = 0.0f);
    IMGUI_API void**    GetVoidPtrRef(ImGuiID txt_License, void* default_val = NULL);
